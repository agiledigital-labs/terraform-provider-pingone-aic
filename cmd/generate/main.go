...
func main() {
	...
	fmt.Printf("wrote %d journeys, %d scripts, %d nodes, %d oauth2 clients, %d variables, %d secrets, %d managed objects, %d endpoints, %d schedules, %d access rules, %d auth mappings, %d roles → %s\n",
		res.Journeys, res.Scripts, res.Nodes, res.OAuth2Clients, res.Variables, res.Secrets, res.ManagedObjects, res.Endpoints, res.Schedules, res.AccessRules, res.AuthMappings, res.Roles, *out)
}
